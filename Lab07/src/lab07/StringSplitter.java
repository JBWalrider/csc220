...
public class StringSplitter {
	...
	//pre: there is another token to return (throws NoSuchElementException if not)
	//post: returns the next toek without consuming it										//misspells "token" wow really?
	public String peek(){
		checkToken();
		return token;
	}
	...
	private void findNextToken(){
		while (!characters.isEmpty() && Character.isWhitespace(characters.peek())){			//if character is whitespace, then skip and update list
			characters.remove();
		}
		if (characters.isEmpty()){															//if end of string, token is null
			token = null;
		}else{
			token = "" + characters.remove();												//otherwise set token to be next character and update list
			if (!SPECIAL_CHARACTER.contains(token)){										//if token isn't special, it could be multiple digits
				boolean done = false;
				while (!characters.isEmpty() && !done){
					char ch = characters.peek();
					if (Character.isWhitespace(ch) || SPECIAL_CHARACTER.indexOf(ch) >= 0){	//if it is whitespace or special, then stop searching
						done = true;
					}else{
						token = token + characters.remove();								//otherwise add the next digit
					}
				}
			}
		}
